...
export const StaffReportRedShift: React.FC = () => {
  const navigate = useNavigate();
  const { imageUrl, loading: imageLoading } = usePageImage('staff', '/images/Staff.png');
  const [staff, setStaff] = useState<StaffMember[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [redShiftId, setRedShiftId] = useState<number | null>(null);
  ...
  const loadRedShiftId = async () => {
    try {
      const { data, error } = await supabase
        .from('02_admin_register_fd2_operational_shifts')
        .select('id')
        .ilike('shift_name', '%red%')
        .single();

      if (error) throw error;
      if (data) setRedShiftId(data.id);
    } catch (error: any) {
      console.error('Error loading Red Shift ID:', error);
  ...
  const loadStaff = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select('*')
        .eq('operational_shift_id', redShiftId)
    ...
    <MainContent>
      <FlexRow>
        <Column style={{ flex: '1', minWidth: '0' }}>
          <Title>Staff Report - Red Shift</Title>
          <Divider />
          <Paragraph>
            Comprehensive report of all staff members assigned to Red Shift operational schedule.
          </Paragraph>
        </Column>
        <ImageColumn>
          {imageLoading ? (
      ...
      <StaffListSection>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <SubTitle>Red Shift Staff Members</SubTitle>
        ...
        {redShiftId && (
          <InfoBox>
            <strong>Filter:</strong> Showing staff assigned to Red Shift (ID: {redShiftId})
          </InfoBox>
        )}
        ...
        {loading ? (
          ...
          <p>No staff members assigned to Red Shift.</p>
        ) : (
          <StaffTable>
            <thead>
              <tr>

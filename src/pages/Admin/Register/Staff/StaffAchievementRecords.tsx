...
export const StaffAchievementRecords: React.FC = () => {
  ...
  const loadAchievements = async (staffId: number) => {
    setLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke('staff-multi-form-crud', {
        method: 'POST',
        body: {
          action: 'list',
          table: '02_admin_staff_5_achievements',
          staff_id: staffId
        }
      });
      ...
      if (error) {
        ...
        setError('Failed to load achievement records');
        return;
      }
  ...
  const handleEdit = (achievement: Achievement) => {
    setFormData({
      ...
      recognition_details: achievement.recognition_details || ''
    });
    setEditingAchievementId(achievement.achievement_id);
    setShowForm(true);
    setError('');
    setSuccess('');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const requestData = {
        action: editingAchievementId ? 'update' : 'create',
        table: '02_admin_staff_5_achievements',
        data: editingAchievementId ? {
          ...
          recognition_details: formData.recognition_details || null
        } : {
          staff_id: staffId,
          achievement_type: formData.achievement_type || null,
          achievement_date: formData.achievement_date || null,
      ...
      if (error) {
        throw new Error(error.message || 'Failed to save achievement record');
      }

      setSuccess(editingAchievementId ? 'Achievement updated successfully!' : 'Achievement added successfully!');
      
      // Clear form and reload list
      handleCancel();
      await loadAchievements(staffId);
    } catch (error: any) {
      setError(error.message || 'An error occurred while saving the achievement');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (achievementId: number) => {
    ...
    try {
      ...
      setError(error.message || 'An error occurred while deleting the achievement');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <MainContent aria-label="Main content">
      <Section aria-labelledby="staff-achievement-title">
        <div style={{ marginTop: '10px' }}>
          <FlexRow>
            <Column style={{ flex: '1', minWidth: '0' }}>
              <Title id="staff-achievement-title">Staff Achievement Records</Title>
              <Divider aria-hidden="true" />
              <Paragraph>
                Record and celebrate staff achievements, awards, commendations, and notable contributions.
              </Paragraph>
            </Column>
            <ImageColumn>
              {imageLoading ? (
      ...
      <FormSection>
        {staffInfo && (
          <FormHeading>
            Achievement Records For: {staffInfo.name} ({staffInfo.employeeNumber})
          </FormHeading>
        )}
        ...
        {showForm && (
          <FormContainer onSubmit={handleSubmit}>
            <h3 style={{ color: '#1177BB', marginBottom: '15px' }}>
              {editingAchievementId ? 'Edit Achievement Record' : 'Add New Achievement Record'}
            </h3>
            ...
            <FieldRow>
              <FieldColumn>
                ...
                <Input
                  ...
                  placeholder="e.g., Award, Commendation, Recognition"
                  required
                  disabled={loading}
                />
              </FieldColumn>
            ...
            <FieldRow>
              <FieldColumn>
                ...
                <TextArea
                  ...
                  onChange={handleInputChange}
                  placeholder="Enter description of the achievement"
                  disabled={loading}
                />
              </FieldColumn>
            </FieldRow>
            ...
            <FieldRow>
              <FieldColumn>
                <Label htmlFor="recognition_details">Recognition Details</Label>
                <TextArea
                  id="recognition_details"
                  name="recognition_details"
                  value={formData.recognition_details}
                  onChange={handleInputChange}
                  placeholder="Enter recognition details, awarding authority, etc."
                  disabled={loading}
                />
              </FieldColumn>
            </FieldRow>
            ...
            <div style={{ marginTop: '20px' }}>
              <SubmitButton type="submit" disabled={loading}>
                {loading ? 'Saving...' : editingAchievementId ? 'Update Achievement' : 'Add Achievement'}
              </SubmitButton>
            ...
            <tbody>
              {achievements.map((achievement) => (
                <tr key={achievement.achievement_id}>
                  <Td>{achievement.achievement_type || 'N/A'}</Td>
                  <Td>{achievement.achievement_date ? new Date(achievement.achievement_date).toLocaleDateString() : 'N/A'}</Td>
                  <Td>{achievement.description || 'N/A'}</Td>
                  <Td>{achievement.recognition_details || 'N/A'}</Td>
                  <Td>
                    <ActionButton 
                      $variant="edit" 
        ...
        {!showForm && achievements.length === 0 && !loading && (
          <EmptyState>
            No achievement records added yet. Click "Add New Achievement" to get started.
          </EmptyState>
        )}
      </FormSection>
    </MainContent>

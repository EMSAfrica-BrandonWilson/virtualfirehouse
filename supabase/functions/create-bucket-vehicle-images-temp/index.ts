...
Deno.serve(async (req: Request) => {
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for vehicle-images" ON storage.objects FOR SELECT USING (bucket_id = 'vehicle-images');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for vehicle-images" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'vehicle-images');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for vehicle-images" ON storage.objects FOR UPDATE USING (bucket_id = 'vehicle-images');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for vehicle-images" ON storage.objects FOR DELETE USING (bucket_id = 'vehicle-images');`
    ];

-- Migration: add_hierarchical_roles_system_v3
-- Created at: 1759843114

-- Create hierarchical role management system (Fixed version v3)
...
BEGIN
    ...
    IF brandon_user_id IS NOT NULL THEN
        ...
        SELECT COUNT(*), role_name INTO existing_count, current_role
        FROM public.user_roles 
        WHERE user_id = brandon_user_id
        GROUP BY role_name;
        
        -- If user has existing role, update it
        ...
        VALUES 
        (brandon_user_id, brandon_user_id, COALESCE(current_role, 'none'), 'system_admin', 'Initial system admin assignment during migration');
    END IF;
END$$;;
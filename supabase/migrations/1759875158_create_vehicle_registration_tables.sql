-- Migration: create_vehicle_registration_tables
-- Created at: 1759875158
...
CREATE TABLE vehicles (
    id SERIAL PRIMARY KEY,
    call_sign_id INTEGER,
    vehicle_type_id INTEGER,
    vehicle_make_id INTEGER,
    vehicle_model VARCHAR(255),
    model_year INTEGER,
    vehicle_age INTEGER,
    registration_plate_number VARCHAR(255) UNIQUE,
    mms_number VARCHAR(255) UNIQUE,
    gate_pass_number VARCHAR(255) UNIQUE,
    gate_pass_expiry_date DATE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    created_by UUID,
    updated_by UUID

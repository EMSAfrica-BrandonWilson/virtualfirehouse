-- Migration: Create Fire Station Menu and Equipment Management System
-- Created: 2025-10-29
...
CREATE TABLE user_fire_station_menu_items (
    ...
    fire_station_id UUID NOT NULL REFERENCES fire_stations_vfh(id) ON DELETE CASCADE,
    menu_item_name VARCHAR(255) NOT NULL,
    created_by_user_id UUID NOT NULL REFERENCES auth.users(id),
    created_by_staff_id UUID NOT NULL REFERENCES staff_basic_info(id),
    department_id UUID NOT NULL REFERENCES emergency_departments(id),
...
CREATE TABLE menu_item_audit_log (
    ...
    old_values JSONB,
    new_values JSONB,
    changed_by_staff_id UUID NOT NULL REFERENCES staff_basic_info(id),
...
WHERE fs.status = 'active' AND ufsmi.is_active = TRUE;
...
WITH CHECK (
    EXISTS (
        ...
        JOIN user_profiles up ON sbi.user_id = up.user_id
        WHERE sbi.id = auth.uid()
        AND up.role IN ('administrator', 'system_admin')
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM staff_basic_info sbi
        WHERE sbi.id = auth.uid()
        AND sbi.department_id = user_fire_station_menu_items.department_id
    )
);
...
USING (
    EXISTS (
        SELECT 1 FROM staff_basic_info sbi
        JOIN emergency_departments ed ON sbi.department_id = ed.id
        JOIN fire_stations_vfh fs ON fs.department_id = ed.id
        WHERE sbi.id = auth.uid()
...
USING (
    EXISTS (
        ...
        JOIN emergency_departments ed ON sbi.department_id = ed.id
        JOIN fire_stations_vfh fs ON fs.department_id = ed.id
        JOIN fire_station_rooms fsr ON fsr.fire_station_id = fs.id
        WHERE sbi.id = auth.uid()
        AND room_equipment.room_id = fsr.id
    )
);
...
CREATE OR REPLACE FUNCTION get_fire_station_equipment_summary(f_station_id UUID)
RETURNS TABLE (
...
COMMENT ON FUNCTION get_fire_station_equipment_summary(UUID) IS 'Get equipment summary for a specific fire station';
